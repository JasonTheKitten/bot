...
public class MusicAdapter implements IAdapter {
	...
	public int getVolume() {
		synchronized(scheduler) {return scheduler.getVolume();}
	}
	public void setVolume(int volume) {
		synchronized(scheduler) {scheduler.setVolume(volume);}
	}
}
	...
	public void playNext() {
		synchronized(this) {
			if (queue.isEmpty()&&playingRadio) {
				...
				MusicUtil.lookup(radio[(int) Math.round(Math.random()*(radio.length-1))]).doOnNext(track->{
					//TODO: Better Music Algorithm
					queue.add(track);
