	...
	@Override public void execute(Message message, MessageAdapter adapter, String argument) {
		adapter.getChannelAdapter(cadapter->
			adapter.getMemberAdapter(madapter->{
				madapter.hasPermission(Permission.KICK_MEMBERS, perm->{
					if (!perm) {
					...
					Promise promise = new Promise(()->{
						if ((Boolean) success.object) {
							adapter.formatTextLocale(LocalizedString.KickSuccess, str->cadapter.send(str));
						}
						else adapter.formatTextLocale(LocalizedString.KickFail, str->cadapter.send(str));
					});
