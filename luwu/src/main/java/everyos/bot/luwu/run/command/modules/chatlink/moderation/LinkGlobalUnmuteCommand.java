...
public class LinkGlobalUnmuteCommand extends CommandBase {
	
	public LinkGlobalUnmuteCommand() {
		super("command.link.unmute.global", e -> true, ChatPermission.SEND_MESSAGES, ChatPermission.NONE);
	}
		...
		return channel
			...
			.flatMap(info -> {
				return user.as(LinkUser.typeWith(info.getLinkID()))
					.flatMap(linkUser -> linkUser.edit(spec -> spec.setMuted(false)))
					.then(channel.getInterface(ChannelTextInterface.class).send(locale.localize("command.link.unmute.global.success")))
					.then(info.getLink())
					.flatMap(link -> link.sendSystemMessage(locale.localize("command.link.unmute.global.system", "user", user.getHumanReadableID())));
			})
			.then();
	}
