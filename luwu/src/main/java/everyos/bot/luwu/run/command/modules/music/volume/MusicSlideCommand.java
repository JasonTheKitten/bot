...
public class MusicSlideCommand extends GenericMusicCommand {
	private boolean isVolumeUp;

	public MusicSlideCommand(boolean isVolumeUp) {
		super(isVolumeUp?"command.music.volume.up":"command.music.volume.down", e->true,
	...
	private Mono<Void> runCommand(Channel channel, MusicManager manager, int slide, Locale locale) {
		return Mono.defer(()->{
			int volume = manager.getVolume()+slide*(isVolumeUp?1:-1);
