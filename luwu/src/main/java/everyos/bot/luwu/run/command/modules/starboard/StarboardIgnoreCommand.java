...
public class StarboardIgnoreCommand extends CommandBase {
	public StarboardIgnoreCommand() {
		super("command.starboard.ignore", e->true,
	...
	private Mono<Void> runCommand(Channel channel, ChannelID channelID, Locale locale) {
		return channel.getServer()
			...
			.flatMap(c->c.edit(spec->{
				spec.ignoreChannel(channelID);
			}))
			.then(channel.getInterface(ChannelTextInterface.class).send("command.starboard.ignore.message"))
			.then();
	}
}