...
public class InviteCommand extends CommandBase {
	public InviteCommand() {
		super("command.invite", e->true, ChatPermission.SEND_MESSAGES, ChatPermission.NONE);
	}
	...
	private Mono<String> getDonateURL(CommandData data) {
		...
		String url = data.getBotEngine().getConfiguration().getCustomField("invite-url");
		return Mono.just(url==null?"(Misconfiguration)":url);
	}

	private Mono<Void> sendDonateURL(Channel channel, String url, Locale locale) {
		return
			channel.getInterface(ChannelTextInterface.class).send(locale.localize("command.invite.message", "url", url))
			.then();
	}
}
