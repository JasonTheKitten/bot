...
public class LinkOptUserCommand extends CommandBase {
	
	public LinkOptUserCommand() {
		super("command.link.optuser", e->true, ChatPermission.SEND_MESSAGES, ChatPermission.NONE);
	}
	...
	private Mono<Void> runCommand(Channel channel, User user, Locale locale) {
		return channel
			...
			.flatMap(info -> user.as(LinkUser.typeWith(info.getLinkID())))
			.flatMap(linkUser -> linkUser.edit(spec -> spec.setOpted(true)))
			.then(channel.getInterface(ChannelTextInterface.class).send(locale.localize("command.link.optuser.message")))
			.then();
	}
